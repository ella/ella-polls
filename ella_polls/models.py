...
class QuizBox(Box):
    def get_context(self):
        ...
        from ella_polls.views import QuestionForm
...
class Quiz(BasePoll, Publishable):
    ...
    def get_result(self, points):
        ...
        """
        # FIXME if there is not found Result, return first possible Result as
...
class Choice(models.Model):
    ...
    def get_percentage(self):
        """
        Compute and return percentage representation of choice object to its
        question's total votes
        """
...
class Survey(Question):
    ...
    def check_vote_by_ip_address(self, ip_address):
        treshold = datetime.fromtimestamp(time.time() - polls_settings.IP_VOTE_TRESHOLD)
        return SurveyVote.objects.filter(survey=self, ip_address=ip_address,
            time__gte=treshold).count() > 0
